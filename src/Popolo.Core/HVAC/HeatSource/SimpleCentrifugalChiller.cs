...
namespace Popolo.Core.HVAC.HeatSource
{
  ...
  public class SimpleCentrifugalChiller : ICentrifugalChiller, IReadOnlyCentrifugalChiller
  {
      ...
      double chilledWaterFlowRate, bool hasInverter)
    {
      ...
      this.theoreticalCOP = (chilledWaterOutletTemperature + 273.15)
      ...
      double coolingWaterFlowRate, double chilledWaterFlowRate)
    {
      ...
      //冷却水出口温度の計算
      double tcho = ChilledWaterOutletTemperature + 273.15;
      double tcdi = coolingWaterInletTemperature + 273.15;
      ...
      //2次方程式の解の公式
      CoolingWaterOutletTemperature = (-bcd - Math.Sqrt(bcd * bcd - 4 * acd * ccd)) / (2d * acd) - 273.15;
