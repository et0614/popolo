...
namespace Popolo.Core.Tests.HVAC.VRF
{
  ...
  public class VRFSystemTests
  {
    ...
    private static (VRFSystem vrf, VRFUnit[] iHexes) MakeCoolingOnlySystem()
    {
      ...
      vrf.Cooling.MaxEvaporatingTemperature = 18;
      return (vrf, iHexes);
    }
    ...
    [Fact]
    public void Constructor_CoolingOnly_CoolingNominalCapacityIsNegative()
    {
      var (vrf, _) = MakeCoolingOnlySystem();
      Assert.True(vrf.Cooling.NominalCapacity < 0,
          $"Cooling.NominalCapacity={vrf.Cooling.NominalCapacity:F2} kW < 0");
    }

    /// <summary>ヒートポンプ機の Heating.NominalCapacity が正値。</summary>
    [Fact]
    public void Constructor_HeatPump_HeatingNominalCapacityIsPositive()
    {
      var (vrf, _) = MakeHeatPumpSystem();
      Assert.NotNull(vrf.Heating);
      Assert.True(vrf.Heating!.NominalCapacity > 0,
          $"Heating.NominalCapacity={vrf.Heating.NominalCapacity:F2} kW > 0");
    }

    /// <summary>冷房専用機は Heating が null。</summary>
    [Fact]
    public void Constructor_CoolingOnly_HeatingIsNull()
    {
      var (vrf, _) = MakeCoolingOnlySystem();
      Assert.Null(vrf.Heating);
    }
    ...
    public void UpdateState_CoolingRated_EvpTempWithinRange()
    {
      ...
      Assert.True(vrf.EvaporatingTemperature <= vrf.Cooling.MaxEvaporatingTemperature,
          $"EvpTemp={vrf.EvaporatingTemperature:F1}°C <= Max={vrf.Cooling.MaxEvaporatingTemperature:F1}°C");
    }
    ...
    {
      var (vrf, iHexes) = MakeCoolingOnlySystem();
      vrf.Cooling.MinEvaporatingTemperature = 5;
